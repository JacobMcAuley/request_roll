class RequestRoll extends FormApplication {
    ...
    _filter(formData, wordReq) {
        ...
        keys.forEach((key) => {
            if (wordReq == "player" || wordReq == "modifier") dict[key.replace(wordReq + "-", "")] = formData[key];
            else {
    ...
    _determineUserSize() {
        ...
        let userSet = [];
        users.forEach((user) => {
            if (!user.isGM || this._showDM) userSet.push(user);
        });
        return userSet;
    }

    _determineOwnedOrActive(userSet) {
        if (this.owned) return this._owned(userSet);
        else return this._active(userSet);
    }
